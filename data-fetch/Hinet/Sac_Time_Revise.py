...
if(not len(sys.argv)==4):
	...
	print('e.g. python Sac_Time_Revise.py ./SAC SAC rSAC')
	sys.exit('\n')
...
for path in sorted( glob.glob('%s/*'%(input_dir)) ):
	...
	# Read event information from CMTSOLUTION
	...
	mag=max(float(cmt[10]),float(cmt[11]))
	evla=cmtfile[4].split(None);evla=evla[1]
	evlo=cmtfile[5].split(None);evlo=evlo[1]
	evdp=cmtfile[6].split(None);evdp=evdp[1]
	evel=0
